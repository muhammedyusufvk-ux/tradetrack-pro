...
const exportPDF = (outlets, collections, sales) => {
  ...
  }).sort((a, b) => { const order = ["A+","A","B","C","D","F"]; return order.indexOf(a.grade.grade) - order.indexOf(b.grade.grade); });
  ...
  <style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Segoe UI',Arial,sans-serif;background:#fff;color:#111;font-size:12px}.header{background:linear-gradient(135deg,#1e1b4b,#312e81);color:#fff;padding:28px 32px}.header h1{font-size:26px;font-weight:800}.summary{display:flex;gap:16px;padding:20px 32px;background:#f8f8ff;border-bottom:2px solid #e5e5f0}.summary-box{flex:1;background:#fff;border-radius:8px;padding:12px 16px;border:1px solid #e5e5f0;text-align:center}.summary-box .val{font-size:18px;font-weight:800;color:#312e81}.summary-box .lbl{font-size:10px;color:#888;text-transform:uppercase}table{width:100%;border-collapse:collapse}th{background:#1e1b4b;color:#fff;padding:10px 12px;font-size:11px;text-align:left;font-weight:600}td{padding:9px 12px;border-bottom:1px solid #f0f0f8;font-size:11.5px}.grade-badge{display:inline-block;padding:3px 10px;border-radius:20px;font-weight:800;font-size:13px;color:#fff;min-width:36px;text-align:center}.footer{padding:20px 32px;font-size:10px;color:#aaa;border-top:1px solid #e5e5f0;margin-top:12px;display:flex;justify-content:space-between}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}</style>
  </head><body>
  <div class="header"><h1>◈ TradeTrack Pro</h1><p style="font-size:12px;opacity:0.7;margin-top:4px">Glassware & Ceramics — Outlet Performance Report</p><div style="font-size:11px;opacity:0.6;margin-top:8px">Generated: ${new Date().toLocaleDateString("en-US",{weekday:"long",year:"numeric",month:"long",day:"numeric"})}</div></div>
  <div class="summary">
  ...
  <table><thead><tr><th>#</th><th>Outlet Name</th><th>Area</th><th>Contact</th><th>Total Sales</th><th>Collected</th><th>Outstanding</th><th>Last Payment</th><th>Grade</th><th>Status</th></tr></thead>
  <tbody>${rows.map((o,i)=>`<tr><td style="color:#888">${i+1}</td><td><strong>${o.name}</strong></td><td>${o.area||"—"}</td><td>${o.contact||"—"}</td><td style="color:#1d4ed8;font-weight:600">${fmtN(o.totalSales)}</td><td style="color:#16a34a;font-weight:600">${fmtN(o.totalCollected)}</td><td style="${o.totalDue>0?"color:#dc2626":"color:#16a34a"};font-weight:600">${fmtN(o.totalDue)}</td><td>${o.lastPayment}</td><td><span class="grade-badge" style="background:${gradeColors[o.grade.grade]}">${o.grade.grade}</span></td><td style="color:${o.grade.color};font-weight:600">${o.grade.label}</td></tr>`).join("")}</tbody></table>
  <div class="footer"><span>TradeTrack Pro — Confidential</span><span>Generated: ${new Date().toLocaleString()}</span></div>
  </body></html>`;
...
export default function App() {
  ...
  useEffect(() => {
    const o = load(SK.outlets);
    const s = load(SK.sales);
    const c = load(SK.collections);
    const t = load(SK.targets);
    if (o) setOutlets(o);
    if (s) setSales(s);
  ...
  const exportCSV = () => {
    ...
    outlets.forEach(o => { const s = getStatus(o, collections); rows.push([o.name, o.area || "", o.contact || "", o.totalDue || 0, s]); });
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#080810", color: "#e2e0f0", fontFamily: "'Sora', sans-serif", paddingBottom: 80 }}>
      ...
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "16px" }}>
          ...
          <div>
            ...
            {areas.length > 1 && (
              <div style={{ display: "flex", gap: 6, marginBottom: 14, overflowX: "auto", paddingBottom: 4 }}>
                {areas.map(a => <button key={a} onClick={() => setAreaFilter(a)} style={{ background: areaFilter === a ? "#7c3aed" : "#1a1a2e", border: "none", color: areaFilter === a ? "#fff" : "#6b7280", borderRadius: 20, padding: "5px 12px", fontSize: 12, cursor: "pointer", whiteSpace: "nowrap" }}>{a}</button>)}
              </div>
            )}
            ...
            {filteredOutlets.map(o => <OutletCard key={o.id} outlet={o} collections={collections} fmt={fmt}
              onEdit={() => { setEditing(o); setModal("outlet"); }}
              onDelete={() => { setOutlets(p => p.filter(x => x.id !== o.id)); setCollections(p => p.filter(c => c.outletId !== o.id)); setSales(p => p.filter(s => s.outletId !== o.id)); showToast("Outlet deleted"); }}
                    ...
                    return (
                      <div key={o.id} style={{ background: m.bg, borderRadius: 10, padding: "12px 14px", marginBottom: 8, border: `1px solid ${m.border}` }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                          <div>
                            <div style={{ fontWeight: 600, fontSize: 14 }}>{o.name}</div>
                            <div style={{ fontSize: 11, color: "#6b7280", marginTop: 3 }}>{o.area && `📍 ${o.area} · `}{last ? `Last payment ${days}d ago` : "Never paid"}</div>
                          </div>
                          <div style={{ textAlign: "right" }}>
                        ...
                        {o.contact && <button onClick={() => whatsapp(o)} style={{ marginTop: 8, background: "#1a3a2a", border: "1px solid #14532d", color: "#4ade80", borderRadius: 6, padding: "5px 12px", fontSize: 11, cursor: "pointer" }}>💬 WhatsApp Reminder</button>}
                        {o.mapsUrl && <button onClick={() => window.open(o.mapsUrl, "_blank")} style={{ marginTop: 8, marginLeft: o.contact ? 6 : 0, background: "#0f2030", border: "1px solid #1e3a5f", color: "#60a5fa", borderRadius: 6, padding: "5px 12px", fontSize: 11, cursor: "pointer" }}>📍 Open in Maps</button>}
                      </div>
                    );
                  })}
                </div>
        ...
        {tab === "Reports" && <Reports outlets={outlets} sales={sales} collections={collections} fmt={fmt} onExport={exportCSV} onExportPDF={() => exportPDF(outlets, collections, sales)} />}
      </div>

      {modal === "outlet" && <OutletModal outlet={editing} onClose={() => { setModal(null); setEditing(null); }} onSave={(data) => {
        if (editing) { setOutlets(p => p.map(o => o.id === editing.id ? { ...o, ...data } : o)); showToast("Outlet updated!"); }
        else { setOutlets(p => [...p, { id: Date.now().toString(), createdAt: new Date().toISOString(), totalDue: 0, ...data }]); showToast("Outlet added!"); }
      ...
      {modal === "sale" && <SaleModal outlets={outlets} preSelected={editing?.id} onClose={() => { setModal(null); setEditing(null); }} onSave={(data) => {
        ...
        showToast("Sale recorded!"); setModal(null); setEditing(null);
      }} />}
      ...
      {modal === "collection" && <CollectionModal outlets={outlets} preSelected={editing?.id} onClose={() => { setModal(null); setEditing(null); }} onSave={(data) => {
        ...
        showToast("Collection recorded!"); setModal(null); setEditing(null);
      }} />}
...
function Reports({ outlets, sales, collections, fmt, onExport, onExportPDF }) {
  const months = useMemo(() => {
    const map = {};
    collections.forEach(c => { const m = c.date?.slice(0, 7); if (m) map[m] = (map[m] || 0) + c.amount; });
  ...
  const graded = useMemo(() => outlets.map(o => ({ ...o, g: getGrade(o, collections, sales) })).sort((a, b) => { const order = ["A+","A","B","C","D","F"]; return order.indexOf(a.g.grade) - order.indexOf(b.g.grade); }), [outlets, collections, sales]);
  const gradeCounts = useMemo(() => { const c = { "A+": 0, "A": 0, "B": 0, "C": 0, "D": 0, "F": 0 }; graded.forEach(o => c[o.g.grade]++); return c; }, [graded]);
  return (
    <div>
      ...
      <div style={{ background: "#0d0d1f", borderRadius: 12, padding: 14, marginBottom: 14, border: "1px solid #1a1a35" }}>
        ...
        {graded.slice(0, 8).map(o => (
          <div key={o.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "7px 0", borderBottom: "1px solid #111120" }}>
            <div><span style={{ fontSize: 13, fontWeight: 600 }}>{o.name}</span>{o.area && <span style={{ fontSize: 11, color: "#4b5563", marginLeft: 6 }}>{o.area}</span>}</div>
...
function SaleModal({ outlets, preSelected, onClose, onSave }) {
  ...
  return (
    <BottomModal title="Record New Sale" onClose={onClose}>
      ...
      <FInput label="Items / Description" value={f.items} onChange={set("items")} placeholder="e.g. 50 glass sets" />
